...
class Upscaler(nn.Module):
    def __init__(self, upscale_factor=2, dtype=torch.float16):
        super(Upscaler, self).__init__()
        self.upscale_factor = upscale_factor

        # Convolutional layer
        self.conv = nn.Conv2d(3, 16, kernel_size=3, padding=1, dtype=dtype)

        # Transposed convolutional layer for upsampling
        self.upconv = nn.ConvTranspose2d(16, 3, kernel_size=2 * upscale_factor, stride=upscale_factor,
                                         padding=upscale_factor // 2, dtype=dtype)
